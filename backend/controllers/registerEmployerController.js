...
export const registerEmployerPost = async (req, res, next) =>{
    const { username, email, password, phoneNumber, companyName, companyWebsite } = req.body;
    ...
    try {
      ...
      try {
          foundEmployerPhone = await UserEmployer.findOne({ phoneNumber: phoneNumber });
        } catch {
          return next(
            createError(500, "Database could not be queried. Please try again")
          );
        ...
        try {
            foundEmployerCompanyName = await UserEmployer.findOne({ companyName: companyName });
          } catch {
            return next(
              createError(500, "Database could not be queried. Please try again")
            );
          ...
          try {
              foundEmployerCompanyWebsite = await UserEmployer.findOne({ companyWebsite: companyWebsite });
            } catch {
              return next(
                createError(500, "Database could not be queried. Please try again")
              );
